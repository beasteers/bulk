...
OUTPUT_COLUMNS = ['deployment_id', 'start_time', 'end_time', 'label', 'annotation_session', 'annotated_by']
...
def get_filter_mask(filt, df):
    ...
    if isinstance(filt, GroupFilter):
        return np.asarray(df[filt.column_name]) == filt.group
...
def bulk_time(path, data_path, labels=None, download=True):
    ...
    def bkapp(doc):
        ...
        if 'annotation_session' not in event_df.columns:
            event_df['annotation_session'] = 'none'
        event_df['annotation_session'] = event_df['annotation_session'].fillna('none')
        event_df.loc[:1000, 'annotation_session'] = 'old'
        if labels is None:
        ...
        def save_df(mod_df):
            ...
            print("fake saving", mod_df.shape)
            mod_df[OUTPUT_COLUMNS].to_csv("tmp.csv")
        ...
        def refresh_tabs(attr, old, new):
            ...
            df = df.loc[i]
            tab_group.tabs = [
                TabPanel(child=get_time_series_tab(data_df, fdf, labels, save), title=f'{label} ({len(fdf)})')
        ...
        active_view.filter.booleans = event_df.label == '?'
        ...
        options, p = embedding_plot(event_df, source, filter_view, active_view, colormap, update_selection)
...
def get_time_series_tab(df, filt_df, labels, save, page_size=20):
    c = column(
        sizing_mode='stretch_width',
        max_height=1000,
        height=1000,
    ...
    def set_page(new):
        ...
        page = min(max(0, new), n_total-1)
        page_df = filt_df.iloc[page:page+page_size].copy()
        refresh_plots()
    ...
    def refresh_plots():
        c.children = [
            ...
            for _, event in page_df.iterrows()
        ]
    ...
    return column(
        row(save_button, prev_button, next_button, change_all_select),
        c
    )
...
def embedding_plot(df, source, view, active_view, colormap, update_selection):
